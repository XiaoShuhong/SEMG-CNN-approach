...
# output: channels - list of length 32, each being data for a channel
def extract_channel(sample):
    channels = [[], [], [], [], [], [], [], [], [], [],
                [], [], [], [], [], [], [], [], [], [],
                [], [], [], [], [], [], [], [], [], [], [], []] # 32th channel is an indicator 
    for line in sample:
        for i in range(32):
...
def extract_channel_from_log(sample):
    channels = [[], [], [], [], [], []]
    for line in sample:
...
def pre_training(fs, t_win, t_step, t_win_decision, files, movement_names=['Rest', 'Fist', 'Open'], plot_it=[True, True, True], train_test_in_diff_file=False, use_reference=False, tree_depth=12, test_size=0.95):
    ...
    open_data = []
    for i in range(1, 11):
        rest_data.append(df_from_csv("datacache/rest/rest"+str(i)+".csv"))
        fist_data.append(df_from_csv("datacache/fist/fist"+str(i)+".csv"))
        open_data.append(df_from_csv("datacache/stretch/stretch"+str(i)+".csv"))
    all_data = [rest_data, fist_data, open_data]
    ...
    for label in range(len(movement_names)):
        ...
        for file_idx in range(len(files[label])): 
            ...
            for i in range(1): 
                if label == 0: channels = rest_data[file_idx]
                elif label == 1: channels = fist_data[file_idx]
                else: channels = open_data[file_idx]
                ...
                channels[:,-3] = np.array([1000 if sample_indicator > 0 else 0 for k in range(len(channels[:,0]))])
                ...
                channels[:,-1] = movement # final label
                ...
                train_channels[:,-3] = mean_ref
                train_channels[:,-2] = std_ref
                train_channels[:,-1] = movement # final label
    ...
    if plot_it[1]:
        ...
        for label in range(len(movement_names)):
            ...
            plt.plot(filtered[:,1:-3]) # 5 active channels 
            # plt.plot(filtered[:,-3]*0.50) # sample indicator
            plt.plot(filtered[:,-2]/2, 'blue') # sample feature
            plt.plot(filtered[:,-1]*200, 'black') # movement label
...
def main():
    ...
    #          ["fist_test.log"], 
    #          ["open_test.log"]] 
    # files = [["rest1.csv", "rest2.csv", "rest3.csv",
    #         #   "rest4.csv", "rest5.csv", "rest6.csv",
    #           "rest7.csv", "rest8.csv", "rest9.csv", "rest10.csv"],
    #          ["fist1.csv", "fist2.csv", "fist3.csv",
    #         #   "fist4.csv", "fist5.csv", "fist6.csv",
    #           "fist7.csv", "fist8.csv", "fist9.csv", "fist10.csv"],
    #          ["stretch1.csv", "stretch2.csv", "stretch3.csv",
    #         #   "stretch4.csv", "stretch5.csv", "stretch6.csv",
    #           "stretch7.csv", "stretch8.csv", "stretch9.csv", "stretch10.csv"]]
    files = [["rest1.csv", "rest2.csv", "rest3.csv"],
             ["fist1.csv", "fist2.csv", "fist3.csv"],
             ["stretch1.csv", "stretch2.csv", "stretch3.csv"]]
    movement_names = ['Rest', 'Fist', 'Open']

    bst = pre_training(fs, t_win, t_step, t_win_decision, files, movement_names, plot_it=[False, True, False], 
                        train_test_in_diff_file=False, use_reference=False, tree_depth=6, test_size=0.2)
    bst.save_model("model_xgboost.json")

    # unit testing
    data_win_x = np.ones((int(t_win*fs), 32))
